...
def send_cmd():
    ...
    try:
        value = float(p.depth_control.text())
        value = int(value).to_bytes(2, byteorder='big', signed=False)
        p.serial1.write(value)

    except ValueError as err:
        print(err)
    # d = int(value).to_bytes(2, byteorder='big', signed=False)
    # self.serial_send(self.serial1, d)
...
def update():
    ...
    # data = serial_listen(serial0, 4)
    # ccl, = struct.unpack('f', data)

    data = serial_listen(p.serial0, size=25)
    cur_depth, prev_depth_raw, cur_temp, depth_update_rate, desired_depth, ccl, automatic = struct.unpack('ffffffb',
                                                                                                          data)
    ...
    if my_thread.isRunning():
        ...
        elif automatic ==1 and (my_thread.vr.depth_control != prev_depth_control) and not my_thread.vr.depth_control is None:
            prev_depth_control = cur_dctrl
            val = int(cur_dctrl[0])
            val = val.to_bytes(2, byteorder='big', signed=False)
            p.serial1.write(val)
...
perf1_tb = QtGui.QLineEdit(); perf1_label = QtGui.QLabel("""Perf1:""")
perf2_tb = QtGui.QLineEdit(); perf2_label = QtGui.QLabel("""Perf2: """)
...
# win.addItem()
# win.nextRow()
# win.setWindowTitle("Example Log")
#
# p = win.addPlot(labels={'bottom': ('Depth'), 'left': 'CCL'})
# p.setWindowTitle('Real-Time CCL Data')
# p.setXRange(0, window_buffer)
# p.setYRange(-1.5, 1.5)
# p.resize(900, 600)
#
# win.nextRow()
#
# p2 = win.addPlot(labels={'bottom': 'Depth', 'left': 'Temperature (F)'})
# p2.setXRange(0, window_buffer)
# p2.setYRange(77, 180)
# p2.resize(900, 600)
